...
本方向不把 Sites Relay 变成以下服务：

- 无需登录即可使用的公共匿名代理
- 可供任意客户端连接的 HTTP `CONNECT` 或 SOCKS 正向代理
- 可访问 loopback、内网、链路本地地址、云元数据或其他非公网服务的 SSRF 跳板
- 将目标网站 HTML 或 JavaScript 直接放到 Sites origin 中执行的内容代理
- 将当前 `/api/proxy/*` 的固定上游和路由白名单替换为客户端目标 URL
